...
int get_pmt_pid(char *dmxdev, int sid)
{
	...
	if(!patfd && (patfd = open(dmxdev, O_RDWR)) < 0) {
		...
		if (rec_psi)
			...
			if ((pmtfd = open(dmxdev, O_RDWR)) < 0) {
				...
				return FALSE;
                        ...
                        /*
                        if ((pidfd = open(dmxdev, O_RDWR)) < 0) {
				perror("opening pid demux failed");
				close(patfd);
				close(audiofd);
				close(dmxfda);
				close(dmxfdv);
...
again:
	...
	while (!feof(cfp))
		...
		if (fec == -1)        fec = FEC_AUTO;
		if (modulation == -1) modulation = QPSK;
		if (delsys == -1)     delsys = SYS_DVBS;
		if (rolloff == -1)    rolloff = ROLLOFF_35;
...
int main(int argc, char *argv[])
{
	...
	unsigned int adapter = 0, frontend = 0, demux = 0, dvr = 0, rec_psi = 1;
	int bypass = 0;
	int opt, copt = 0;
